...
PCA_all_astragali <- readRDS("data/astragali_pca_results_2025-10-29.rds")
all_astragali_info <- read.csv("data/modern_astragali_post_gm_2025-10-29.csv")
...
((cva_1v2_bs | cva_1v2_95 | cva_1v2_all_pcs) / (cva_1v3_bs | cva_1v3_95 | cva_1v3_all_pcs)) +
...
```{r, fig.height=12, fig.width=18}
cva_sp_bs_1v2 + cva_sp_95_1v2 + cva_sp_1v2 + guide_area() + cva_sp_95_1v3 + cva_sp_1v3  + plot_layout(nrow = 2, guides = "collect")
ggsave("figures/cva_species_means_comparison.png", height = 12, width = 18)
```

## CVA: species means, slightly reducing the number of PCs?
Try to get rid of multicollinearity issue

### PCs 1-34?
```{r}
#conduct the CVA (without cross-validation):
cvaresults_sp_34 <- lda(species_PC_scores_scaled[,1:34], cvagrps_sp, CV=FALSE)
A_sp_34 <- cvaresults_sp_34$scaling
grpmeans_sp_34 <- cvaresults_sp_34$means

#project specimens onto LD1
C_sp_34 <- species_PC_scores_scaled[,1:34]%*%A_sp_34

#make this easier to visualize
C_sp_34_info <- data.frame(LD1 = C_sp_34[,1],
                     LD2 = C_sp_34[,2],
                     LD3 = C_sp_34[,3],
                     Habitat = species_df$Habitat,
                     Tribe = species_df$Tribe)
...
#conduct CVA with leave-one-out cross-validation of group membership:
cvaresultsCV_sp_34 <- lda(species_PC_scores_scaled[,1:34], cvagrps_sp, CV=TRUE)
crossval_sp_34 <- table(cvagrps_sp, cvaresultsCV_sp_34$class) # confusion matrix of specimen assignments
diag(prop.table(crossval_sp_34, 1)) # percent correct per group
sum(diag(prop.table(crossval_sp_34))) # total percent correct

#for a fancier table
altcrossval_sp_34 <- rbind(crossval_sp_34[1,]/sum(crossval_sp_34[1,]), crossval_sp_34[2,]/sum(crossval_sp_34[2,]), crossval_sp_34[3,]/sum(crossval_sp_34[3,]), crossval_sp_34[4,]/sum(crossval_sp_34[4,]))
dimnames(altcrossval_sp_34) <- list("Actual"=c("F", "HC", "LC", "O"), "Predicted (CV)"=c("F", "HC", "LC", "O"))
print(round(altcrossval_sp_34, 3))

# proportion of variance explained by each CV axis
cvaresults_sp_34$svd^2 / sum(cvaresults_sp_34$svd^2)
```
Still gives multicollinearity warning

### PCs 1-33?
```{r}
#conduct the CVA (without cross-validation):
cvaresults_sp_33 <- lda(species_PC_scores_scaled[,1:33], cvagrps_sp, CV=FALSE)
A_sp_33 <- cvaresults_sp_33$scaling
grpmeans_sp_33 <- cvaresults_sp_33$means

#project specimens onto LD1
C_sp_33 <- species_PC_scores_scaled[,1:33]%*%A_sp_33

#make this easier to visualize
C_sp_33_info <- data.frame(LD1 = C_sp_33[,1],
                     LD2 = C_sp_33[,2],
                     LD3 = C_sp_33[,3],
                     Habitat = species_df$Habitat,
                     Tribe = species_df$Tribe)
...
#conduct CVA with leave-one-out cross-validation of group membership:
cvaresultsCV_sp_33 <- lda(species_PC_scores_scaled[,1:33], cvagrps_sp, CV=TRUE)
crossval_sp_33 <- table(cvagrps_sp, cvaresultsCV_sp_33$class) # confusion matrix of specimen assignments
diag(prop.table(crossval_sp_33, 1)) # percent correct per group
sum(diag(prop.table(crossval_sp_33))) # total percent correct

#for a fancier table
altcrossval_sp_33 <- rbind(crossval_sp_33[1,]/sum(crossval_sp_33[1,]), crossval_sp_33[2,]/sum(crossval_sp_33[2,]), crossval_sp_33[3,]/sum(crossval_sp_33[3,]), crossval_sp_33[4,]/sum(crossval_sp_33[4,]))
dimnames(altcrossval_sp_33) <- list("Actual"=c("F", "HC", "LC", "O"), "Predicted (CV)"=c("F", "HC", "LC", "O"))
print(round(altcrossval_sp_33, 3))

# proportion of variance explained by each CV axis
cvaresults_sp_33$svd^2 / sum(cvaresults_sp_33$svd^2)
```
Same problem.
...
```{r}
#conduct the CVA (without cross-validation):
cvaresults_sp_32 <- lda(species_PC_scores_scaled[,1:32], cvagrps_sp, CV=FALSE)
A_sp_32 <- cvaresults_sp_32$scaling
grpmeans_sp_32 <- cvaresults_sp_32$means

#project specimens onto LD1
C_sp_32 <- species_PC_scores_scaled[,1:32]%*%A_sp_32

#make this easier to visualize
C_sp_32_info <- data.frame(LD1 = C_sp_32[,1],
                     LD2 = C_sp_32[,2],
                     LD3 = C_sp_32[,3],
                     Habitat = species_df$Habitat,
                     Tribe = species_df$Tribe)
...
#conduct CVA with leave-one-out cross-validation of group membership:
cvaresultsCV_sp_32 <- lda(species_PC_scores_scaled[,1:32], cvagrps_sp, CV=TRUE)
crossval_sp_32 <- table(cvagrps_sp, cvaresultsCV_sp_32$class) # confusion matrix of specimen assignments
diag(prop.table(crossval_sp_32, 1)) # percent correct per group
sum(diag(prop.table(crossval_sp_32))) # total percent correct

#for a fancier table
altcrossval_sp_32 <- rbind(crossval_sp_32[1,]/sum(crossval_sp_32[1,]), crossval_sp_32[2,]/sum(crossval_sp_32[2,]), crossval_sp_32[3,]/sum(crossval_sp_32[3,]), crossval_sp_32[4,]/sum(crossval_sp_32[4,]))
dimnames(altcrossval_sp_32) <- list("Actual"=c("F", "HC", "LC", "O"), "Predicted (CV)"=c("F", "HC", "LC", "O"))
print(round(altcrossval_sp_32, 3))

# proportion of variance explained by each CV axis
cvaresults_sp_32$svd^2 / sum(cvaresults_sp_32$svd^2)
```
```{r}
hull_dfa_sp_32_12 <- C_sp_32_info %>% group_by(Habitat) %>%
  slice(chull(LD1, LD2))
cva_sp_32_1v2 <- ggplot()+
  geom_polygon(data = hull_dfa_sp_32_12, aes(x = LD1, y = LD2, color = Habitat, fill = Habitat), linewidth = 1, alpha = 0.2)+
  geom_point(data = C_sp_32_info, mapping = aes(x = LD1, y = LD2, color = Habitat, shape = Tribe), size = 3)+
  scale_color_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"))+
  scale_fill_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"))+
  tribe_shapes() +
  theme_classic(base_size = 22) +
...
hull_dfa_sp_32_13 <- C_sp_32_info %>% group_by(Habitat) %>%
  slice(chull(LD1, LD3))
cva_sp_32_1v3 <- ggplot()+
  geom_polygon(data = hull_dfa_sp_32_13, aes(x = LD1, y = LD3, color = Habitat, fill = Habitat), linewidth = 1, alpha = 0.2)+
  geom_point(data = C_sp_32_info, mapping = aes(x = LD1, y = LD3, color = Habitat, shape = Tribe), size = 3)+
  scale_color_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"))+
  scale_fill_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"))+
  tribe_shapes() +
  theme_classic(base_size = 22) +
...
hull_dfa_sp_32_23 <- C_sp_32_info %>% group_by(Habitat) %>%
  slice(chull(LD2, LD3))
cva_sp_32_2v3 <- ggplot()+
  geom_polygon(data = hull_dfa_sp_32_23, aes(x = LD2, y = LD3, color = Habitat, fill = Habitat), linewidth = 1, alpha = 0.2)+
  geom_point(data = C_sp_32_info, mapping = aes(x = LD2, y = LD3, color = Habitat, shape = Tribe), size = 3)+
  scale_color_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"))+
  scale_fill_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"))+
  tribe_shapes() +
  theme_classic(base_size = 22) +
...
```{r, fig.height=12, fig.width=18}
cva_sp_bs_1v2 + cva_sp_95_1v2 + cva_sp_32_1v2 + guide_area() + cva_sp_95_1v3 + cva_sp_32_1v3  + plot_layout(nrow = 2, guides = "collect")
ggsave("figures/cva_species_means_comparison_v2.png", height = 12, width = 18)
```
...
source(file = "code/phylo.fda.v0.2.R", local = TRUE)
```
...
pdfa_1v2_all <- ggplot()+
  ...
  xlab("DA1")+
  ylab("DA2")+
  ggtitle(expression(paste("PCs 1-35, optimal ", lambda, " = 0.46")))
...
## PFDA: species means, first 32 PCs 
(no multicollinearity here)

```{r}
# select the PCs first
X_32 <- signif(species_PC_scores_scaled[,1:32],4)
```

```{r}
# optimal lambda
optLambda(X_32,habs,zurano_tree)
```

```{r perform the discriminant analysis}
pri  <- c(0.25, 0.25, 0.25, 0.25) # The prior probabilities will vary with your datasets. Replace by equal priors if unknown. (e.g., if 4 groups, just use 0.25 for each if unknown)
optl_32 <- 0 # Replace with the optimal lambda value from above.
pfda_32 <- phylo.fda(X_32, habs, zurano_tree, val = optl_32, treetrans = rescale, priin = pri) # just run using the training data
```
...
```{r leave one out cross validation for pc1-32}
# use un-scaled species mean PC scores for this
X_32_cv <- signif(species_PC_scores[,1:32], 4)

pfda_astrag_cv_32 <- data.frame(matrix(ncol=9, nrow = 0))
colnames(pfda_astrag_cv_32) <- c("true class" ,"predicted class", "P(F)", "P(HC)", "P(LC)", "P(O)", "DA1", "DA2", "DA3")
pfda_astrag_cv_32 <- pfda_astrag_cv_32 %>% mutate_if(is.logical, as.character)

for (i in 1:nrow(X_32_cv)) {
  test_taxon <- rownames(X_32_cv)[i] # which taxon to leave out
  training_taxa <- rownames(X_32_cv)[-i] # which taxa are the training set (remainder)
  x_train <- X_32_cv[-i,] # matrix of data for training taxa
  habs_train <- habs[-i] # habitats of training taxa 
  tree_train <- drop.tip(zurano_tree, test_taxon) # tree minus test taxon
  
  pri_cv  <- c(0.25, 0.25, 0.25, 0.25) # The prior probabilities will vary with your datasets. Replace by equal priors if unknown.
  optl_cv <- optl_32 # Replace with the maximum likelihood lambda 
  pfda_cv <- phylo.fda.pred(dataA=X_32_cv, grpA=habs, taxtaxA=taxa, tretreA=zurano_tree, testlistn=i, val=optl_cv, priin=pri_cv) # i represents the row number of the species left out. could also be a vector of row numbers if needed
  ...
  pfda_astrag_cv_32 <- bind_rows(pfda_astrag_cv_32, test_results) # stick new result onto vector of results
}

crossval_pfda_32 <- table(pfda_astrag_cv_32$`true class`, pfda_astrag_cv_32$`predicted class`)
prop.table(crossval_pfda_32, 1)
diag(prop.table(crossval_pfda_32, 1)) # percent correct per group
sum(diag(prop.table(crossval_pfda_32))) # total percent correct
```


```{r evaluate pFDA performance on training data only}
training.class.32 <- as.character(predict(pfda_32, pfda_32$DATA, type="class"))
training.variates.32 <- predict(pfda_32, pfda_32$DATA, type="variates")
training.prob.32 <- predict(pfda_32, pfda_32$DATA, type="posterior")
training.results.32 <- cbind(as.character(habs), training.class.32, training.prob.32, training.variates.32)
colnames(training.results.32) <- c("true class", "predicted class", "P(F)", "P(HC)", "P(LC)", "P(O)", "DA1", "DA2", "DA3")
rownames(training.results.32) <- taxa
```

```{r plot the results}
training.results.32 <- as.data.frame(training.results.32)
training.results.32$DA1 <- as.numeric(training.results.32$DA1)
training.results.32$DA2 <- as.numeric(training.results.32$DA2)
training.results.32$DA3 <- as.numeric(training.results.32$DA3)
training.results.32 <- cbind(training.results.32, species_df$Tribe)
colnames(training.results.32)[colnames(training.results.32) == "species_df$Tribe"] <- "Tribe"

hull_pfda_12_32 <- training.results.32 %>% group_by(`true class`) %>%
  slice(chull(DA1, DA2))
pdfa_1v2_32 <- ggplot()+
  geom_polygon(data = hull_pfda_12_32, aes(x = DA1, y = DA2, color = `true class`, fill = `true class`), linewidth = 1, alpha = 0.2)+
  geom_point(data = training.results.32, mapping = aes(x = DA1, y = DA2, color = `true class`, shape = Tribe), size = 3)+
  scale_fill_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"), name = "Habitat")+
  scale_color_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"), name = "Habitat")+
  tribe_shapes() +
  theme_classic(base_size = 22) +
  xlab("DA1")+
  ylab("DA2")+
  ggtitle(expression(paste("PCs 1-32, optimal ", lambda, " = 0")))

hull_pfda_13_32 <- training.results.32 %>% group_by(`true class`) %>%
  slice(chull(DA1, DA3))
pfda_1v3_32 <- ggplot()+
  geom_polygon(data = hull_pfda_13_32, aes(x = DA1, y = DA3, color = `true class`, fill = `true class`), linewidth = 1, alpha = 0.2)+
  geom_point(data = training.results.32, mapping = aes(x = DA1, y = DA3, color = `true class`, shape = Tribe), size = 3)+
  scale_fill_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"), name = "Habitat")+
  scale_color_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"), name = "Habitat")+
  tribe_shapes() +
  theme_classic(base_size = 22) +
...
hull_pfda_23_32 <- training.results.32 %>% group_by(`true class`) %>%
  slice(chull(DA2, DA3))
pfda_2v3_32 <- ggplot()+
  geom_polygon(data = hull_pfda_23_32, aes(x = DA2, y = DA3, color = `true class`, fill = `true class`), linewidth = 1, alpha = 0.2)+
  geom_point(data = training.results.32, mapping = aes(x = DA2, y = DA3, color = `true class`, shape = Tribe), size = 3)+
  scale_fill_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"), name = "Habitat")+
  scale_color_brewer(palette = "BrBG", direction = -1, labels = c("Forest", "Heavy Cover", "Light Cover", "Open"), name = "Habitat")+
  tribe_shapes() +
  theme_classic(base_size = 22) +
...
```{r, fig.height=12, fig.width=8}
( pdfa_1v2_32 / pfda_1v3_32 ) + plot_layout(axis_titles = "collect", guides = "collect") 
ggsave("figures/astragalus_pfda_32pcs.png", height = 12, width = 8)
```
...
pdfa_1v2_95 <- ggplot()+
  ...
  xlab("DA1")+
  ylab("DA2")+
  ggtitle(expression(paste("PCs 1-19, optimal ", lambda, " = 0")))
...
pdfa_1v2_bs <- ggplot()+
  ...
  xlab("DA1")+
  ylab("DA2")+
  ggtitle(expression(paste("PCs 1-2, optimal ", lambda, " = 0.13")))
```
...
```{r, fig.height=12, fig.width=18}
pdfa_1v2_bs + pdfa_1v2_95 + pdfa_1v2_all + guide_area() + pfda_1v3_95 + pfda_1v3_all + plot_layout(nrow = 2, guides = "collect")
ggsave("figures/pfda_comparison.png", height = 12, width = 18)
```

```{r, fig.height=12, fig.width=18}
pdfa_1v2_bs + pdfa_1v2_95 + pdfa_1v2_32 + guide_area() + pfda_1v3_95 + pfda_1v3_32 + plot_layout(nrow = 2, guides = "collect")
ggsave("figures/pfda_comparison_v2.png", height = 12, width = 18)
```
