...
astragali_info <- read_csv("out/modern_astragali_post_gm_2025-10-29.csv")
...
(hab_histo | hab_cumulative_prop | hab_log_cumulative_odds)+
  plot_annotation(title = "Empirical distributions of habitat categories")
```
...
predicted_hab_histo_pc_v3 <- ggplot(data = pc_model_v3_predict)+
  geom_histogram(aes(x = Habitat, fill = Predicted_Habitat), stat = "count", binwidth = 1/2)+
  scale_fill_brewer(palette = "BrBG", name = "Predicted\nHabitat")+
  ggtitle("Posterior predictions", subtitle = "PCs 1-8")+
...
pc_model_v3_wsum_plot <- ggplot(pc_model_v3_predict) +
  ...
  ylim(1,4)+
  theme_classic(base_size = 14)+
  ggtitle("PC model", subtitle = "PCs 1-8")
...
predicted_hab_histo_pc_v4 <- ggplot(data = pc_model_v4_predict)+
  geom_histogram(aes(x = Habitat, fill = Predicted_Habitat), stat = "count", binwidth = 1/2)+
  scale_fill_brewer(palette = "BrBG", name = "Predicted\nHabitat")+
  ggtitle("Posterior predictions", subtitle = "PCs 1-53")+
  theme_classic(base_size = 14)
...
pc_model_v4_wsum_plot <- ggplot(pc_model_v4_predict) +
  ...
  ylim(1,4)+
  theme_classic(base_size = 14)+
  ggtitle("PC model", subtitle = "PCs 1-53")
...
```{r}
pc_model_v4 <- add_criterion(pc_model_v4, "waic")
...
predicted_hab_histo_pc_v5 <- ggplot(data = pc_model_v5_predict)+
  geom_histogram(aes(x = Habitat, fill = Predicted_Habitat), stat = "count", binwidth = 1/2)+
  scale_fill_brewer(palette = "BrBG", name = "Predicted\nHabitat")+
  ggtitle("Posterior predictions", subtitle = "PCs 1-30")+
  theme_classic(base_size = 14)
...
Can we assess predictions another way? What about a weighted sum of the probabilities:
...
pc_model_v5_wsum_plot <- ggplot(pc_model_v5_predict) +
  ...
  ylim(1,4)+
  theme_classic(base_size = 14)+
  ggtitle("PC model", subtitle = "PCs 1-30")
...
WAIC comparison:
```{r}
pc_model_v5 <- add_criterion(pc_model_v5, "waic")

compare_models <- loo_compare(pc_model_v2, pc_model_v3, pc_model_v4, pc_model_v5, criterion = "waic")
...
((predicted_hab_histo_pc_v3 | predicted_hab_histo_pc_v4 | predicted_hab_histo_pc_v5) + plot_layout(guides = "collect", axis_titles = "collect")) / ((pc_model_v3_wsum_plot | pc_model_v4_wsum_plot | pc_model_v5_wsum_plot ) + plot_layout(guides = "collect", axis_titles = "collect"))
...
```{r}
hyp_v2 <- "sd_Genus_species_phylo__Intercept^2 / (sd_Genus_species_phylo__Intercept^2 + 3.289868) = 0"
...
phylo_model_v2_wsum_plot <- ggplot(phylo_model_v2_predict) +
  ...
  xlab("known habitat")+
  theme_classic(base_size = 14)+
  ggtitle("Phylo model (v2)", subtitle = "species effect\n+ phylogenetic effect")
...
predicted_hab_histo_phylo_v2 <- ggplot(data = phylo_model_v2_predict)+
  geom_histogram(aes(x = Habitat, fill = Predicted_Habitat), stat = "count", binwidth = 1/2)+
  scale_fill_brewer(palette = "BrBG", name = "Predicted\nHabitat")+
  ggtitle("Posterior predictions", subtitle = "PCs 1-30 + phylogeny")+
  theme_classic(base_size = 14)
...
predicted_hab_histo_phylo_v2_nl <- ggplot(data = phylo_model_v2_predict_nl)+
  ...
  ggtitle("Posterior predictions", subtitle = "PCs 1-30 + phylogeny, without species coefficients")+
  theme_classic(base_size = 14)
